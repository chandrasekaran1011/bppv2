    ...
    <v-container max-width="1400px" class="mx-auto">
        <v-card>
            <v-card-title class="indigo darken-1 white--text">
              <v-btn icon @click="$router.push({name:'ReportIndex'})"><v-icon color="white">fas fa-arrow-left</v-icon> </v-btn> Download Master Report
            </v-card-title>
            <v-card-text>
                <v-row no-gutters class="mt-2">
                    ...
                    <v-col cols="12" :sm="12"  :md="3" class="pa-2">
                        <v-select :items="searchList" :disabled="true" :error-messages="criteriaErrors" @input="$v.criteria.$touch()" @blur="$v.criteria.$touch()" v-model="criteria" item-text="name" item-value="unique" label="Select Criteria" outlined></v-select>
                    </v-col>
                    ...
                    <v-col cols="12" :sm="12" :md="3" class="pa-2">
                        <v-menu ref="datemenu" v-model="datemenu2" :close-on-content-click="false" transition="scale-transition" offset-y max-width="290px" min-width="290px">
                            <template v-slot:activator="{ on }">
                                <v-text-field v-model="keyword" label="Month" readonly :error-messages="keywordErrors"  @blur="$v.keyword.$touch()" persistent-hint prepend-icon="fas fa-calendar" v-on="on"></v-text-field>
                            </template>
...
export default {
    ...
    methods: {
        ...
        search() {
            ...
            axios.post(window.links.masterReport, formData)
                .then(res => {
