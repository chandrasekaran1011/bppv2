    ...
    <v-container max-width="1400px" class="mx-auto">
        <v-card>
            <v-card-title class="indigo darken-1 white--text">
               <v-btn icon @click="$router.push({name:'ReportIndex'})"><v-icon color="white">fas fa-arrow-left</v-icon> </v-btn> Download CDO Report
            </v-card-title>
            <v-card-text>
...
export default {
    ...
    methods: {
        ...
        search() {
            ...
            axios.post(window.links.cdoReport, formData)
                .then(res => {
